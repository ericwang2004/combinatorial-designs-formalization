...
-- def toQuadraticForm (M : Matrix m m α)  :=
--   fun (x : Matrix m (Fin 1) α) ↦ (xᵀ * M * x)

-- theorem equiv_forms_of_matCongr' {M N : Matrix m m α} (h : M ∼ₘ N) :
--     {toQuadraticForm M x | x} ⊆ {toQuadraticForm N x | x} := by
--   rintro _ ⟨x, rfl⟩
--   use h.Aᵀ * x
--   simp only [toQuadraticForm, transpose_mul, transpose_transpose]
--   have : M = h.A * N * h.Aᵀ := by
--     rw [←h.cong, reindexAlgEquiv_apply]
--     sorry
