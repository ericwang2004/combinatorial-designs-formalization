...
noncomputable def multiset_to_function {X : Type*} (ms : Multiset (Finset X)) :
  Fin (Multiset.card ms) → Finset X :=
fun i =>
  -- Convert multiset to list (using some ordering)
  let list_version := ms.toList
  -- Index into the list
  list_version.get ⟨i.val, by
    refine Fin.val_lt_of_le i ?_
    refine Nat.le_of_eq ?_
    exact Eq.symm (Multiset.length_toList ms)
  ⟩
...
lemma card_filter_multiset_eq_card_filter_indexed {X : Type*} [DecidableEq X]
    ...
    #{i : Fin ms.card | p (multiset_to_function ms i)} = (ms.filter p).card := by
  sorry

/-- ### Kramer-Mesner Construction  -/
noncomputable def bibdOfKMCondition {k : ℕ} (v l : ℕ)
    (hX' : Fintype.card X = v) (hvk' : k < v ∧ 2 ≤ k)
    (z : Matrix (Fin 1) {O : jOrbits X G // orbitCard O = k} ℤ) (hz : ∀ O, 0 ≤ z 0 O)
    (km : KramerMesnerCondition X G l z) :
    let b := (Finset.sum (Finset.univ : Finset {O : jOrbits X G // orbitCard O = k})
      (fun O => (z 0 O).natAbs • (orbit_blocks O))).card;
    BIBD X v b k l where
  blocks := by
    let this := Finset.sum (Finset.univ : Finset {O : jOrbits X G // orbitCard O = k})
      (fun O => (z 0 O).natAbs • (orbit_blocks O))
    exact (multiset_to_function this)
  hvk := hvk'
  hX := hX'
  hA := by
    intro i
    unfold multiset_to_function
    simp only [Fin.isValue, List.get_eq_getElem]
    let this := Finset.sum (Finset.univ : Finset {O : jOrbits X G // orbitCard O = k})
      (fun O => (z 0 O).natAbs • (orbit_blocks O))
    have h : ↑i < this.toList.length := by
      exact Fin.val_lt_of_le i (Nat.le_of_eq (Eq.symm (Multiset.length_toList this)))
    have h_mem : this.toList[i] ∈ this := by
      rw [← Multiset.mem_toList]
      exact List.mem_of_getElem rfl
    rw [Finset.mem_sum] at h_mem
    obtain ⟨O, _, h_in_orbit⟩ := h_mem
    have h_in_orbit_blocks : this.toList[i] ∈ orbit_blocks O := by
      exact Multiset.mem_of_mem_nsmul h_in_orbit
    simp only [orbit_blocks, Multiset.mem_filter] at h_in_orbit_blocks
    obtain ⟨_, h_orbit_eq⟩ := h_in_orbit_blocks
    have eq : orbitCard O.val = #(this.toList[i]) := by
      rw [orbitCard, ← h_orbit_eq, Quotient.lift_mk]
      rfl
    rw [O.property] at eq
    dsimp [this] at eq
    exact Eq.symm ((fun {m n} => Int.ofNat_inj.mp) eq)
  balance := by
    intro x y hxy
    let this := Finset.sum (Finset.univ : Finset {O : jOrbits X G // orbitCard O = k})
     (fun O => (z 0 O).natAbs • (orbit_blocks O))
    ...
    simp only [Matrix.mul_apply, A, Matrix.of_apply] at key_eq
    rw [Mathlib.Tactic.Zify.natCast_eq]
    calc ↑(#{i | x ∈ multiset_to_function (∑ O, (z 0 O).natAbs • orbit_blocks O) i ∧
         y ∈ multiset_to_function (∑ O, (z 0 O).natAbs • orbit_blocks O) i} )
        = ↑(#{i | {x, y} ⊆ multiset_to_function (∑ O, (z 0 O).natAbs • orbit_blocks O) i}) := by
          simp only [Fin.isValue, subset_iff, mem_insert, mem_singleton, forall_eq_or_imp,
            forall_eq]
      _ = ↑(this.filter (fun S => {x, y} ⊆ S)).card := by
          congr 1
      ...
      _ = ∑ O, ((z 0 O).natAbs : ℤ) *
          ...
          classical
          have h_nat : (this.filter (fun S : Finset X ↦ {x, y} ⊆ S)).card =
            ...
            simpa [p, this] using h (Finset.univ)
          simpa using congrArg (fun n : ℕ ↦ (n : ℤ)) h_nat
      _ = ∑ O, z 0 O * ↑(countSubsetOrbit X G ↑O ↑O₂) := by
          congr 1
          ext O
          have h_eq : ((z 0 O).natAbs : ℤ) * ↑((orbit_blocks O).filter (fun S => {x, y} ⊆ S)).card =
                      z 0 O * ↑((orbit_blocks O).filter (fun S => {x, y} ⊆ S)).card := by
